...
int acpi_setup_smmu_for_test(struct domain *d, u16 seg, u8 bus, u8 devfn)
{
    ...
    *(u8*) &pci_device.devfn = devfn;
    ...
    pci_device.arch.dev.type = DEV_PCI;
    printk("%d %d %d \n",pci_device.seg,pci_device.bus,pci_device.devfn);

    if (!iort_iommu_configure(device))
        ret = -1;
    ...
    if (device->iommu_fwspec) {
        ...
        ret = arm_smmu_assign_dev(d, 0, device, 0);
    } else {
